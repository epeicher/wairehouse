...
- **Users:** users window enriches core `wp/v2/users` via `register_rest_field` (`includes/users-window/window.php:466`); existing plugin user-meta keys all use a `openstation_` / `_desktop_mode_` prefix; avatars via `get_avatar_url()` server-side and `<os-avatar>` client-side.
